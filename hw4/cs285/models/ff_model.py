...
class FFModel(nn.Module, BaseModel):
    ...
    def forward(
        ...
        ##### my code here ######
        # Note: actually theres norm n unnorm fns in utils
    ...
    def update(self, observations, actions, next_observations, data_statistics):
        ...
        ##### my code here ######
        # Note: actually theres norm n unnorm fns in utils
        s_t_1 = ptu.from_numpy(next_observations)
        s_t = ptu.from_numpy(observations)
        mu = ptu.from_numpy(data_statistics['delta_mean'])
        sigma = ptu.from_numpy(data_statistics['delta_std'])
        target =  (s_t_1 - s_t - mu) / sigma
        _, f = self.forward(s_t,
